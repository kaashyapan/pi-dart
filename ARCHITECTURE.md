...
Matching is conservative, quoted segments are
...
target a specific package/dir), important for multi-package
...
summarized content *and* gets the error framing.
...
**Fresh-checkout detection:** on a project where `dart pub get` has never run (no
`.dart_tool/package_config.json`), analyze reports phantom errors — every `package:` import
fails with `URI_DOES_NOT_EXIST` and every symbol from those packages is `UNDEFINED_*`. Those
arrive as ordinary `COMPILE_TIME_ERROR`s, indistinguishable from genuinely broken imports.
When such failures (and their symbol-level fallout) account for ≥80% of the report, the tool
appends a hint naming the unresolved packages and asking whether `dart pub get` has been run
in this environment, and sets `unresolvedProjectHint: true` in details. The threshold is
deliberately conservative: one typo'd import among real errors stays below it, so mixed
signal is not misattributed to the environment.
